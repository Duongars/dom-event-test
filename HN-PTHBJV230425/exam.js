...
function validateForm() {
    ...
    if (productName.length < 6 || !productName.length > 5) {
...
function creatOredit() {
    ...
    let flagGroup = document.querySelectorAll(".ra");
    // duyệt và lấy các kết quả của input Size in ra mang sizeResult
    let sizeResult = ""
    flagGroup.forEach(function (flag) {
        if (flag.checked == true) {
            // thêm dấu ", " sau mỗi kết quả
            sizeResult += flag.value + ",";
        }
    })
    // loại bỏ dấu "," cuối cùng trong mảng vd(x,xl,l,)
    if (sizeResult[sizeResult.length - 1] == ",") {
        sizeResult = sizeResult.slice(0, -1);
    }
